...
import {
  ...
  CONPTY_UNARMED_READY_TIMEOUT_FACTOR,
  ConPtyHostPool,
  logChildStderr,
...
describe("ConPTY control protocol", () => {
  ...
  it("reports the backend that runs through the reference wrapper", () => {
    expect(
      new RefPsuedoterminal(pseudoterminal({ win32Backend: "legacy" }))
        .win32Backend,
    ).toBe("legacy");
    expect(
      new RefPsuedoterminal(pseudoterminal({ win32Backend: "conpty" })).dup()
        .win32Backend,
    ).toBe("conpty");
    expect(
...
describe("Windows named-pipe ConPTY readiness", () => {
  ...
  /** Reads the promise's rejection, or `PENDING` while it has none. */
  function rejectionOf(promise: Promise<unknown>): () => unknown {
  ...
  it("bounds a channel nobody armed with the fallback deadline", async () => {
    vi.useFakeTimers({ toFake: ["setTimeout", "clearTimeout"] });
    try {
      const channel = await newChannel(20),
        rejection = rejectionOf(channel.ready);
      await vi.advanceTimersByTimeAsync(
        20 * CONPTY_UNARMED_READY_TIMEOUT_FACTOR - 1,
      );
      expect(rejection()).toBe(PENDING);
      // An owner that never arms must not leave a silent host hanging.
  ...
  it("tightens the deadline to the ready timeout once it is armed", async () => {
    vi.useFakeTimers({ toFake: ["setTimeout", "clearTimeout"] });
    try {
      const channel = await newChannel(20),
        rejection = rejectionOf(channel.ready);
      // Source materialization, the environment lookup, and the spawn all run
      // before the host exists; none of it may count against the host.
      await vi.advanceTimersByTimeAsync(30);
      channel.armReadyDeadline();
      await vi.advanceTimersByTimeAsync(19);
      expect(rejection()).toBe(PENDING);
      // Still short of the fallback, so only the armed deadline can fire.
      await vi.advanceTimersByTimeAsync(1);
      await expect(channel.ready).rejects.toMatchObject({ reason: "timeout" });
      // The timeout tore the channel down; arming again must not revive it.
      channel.armReadyDeadline();
      expect(vi.getTimerCount()).toBe(0);
    } finally {
      vi.useRealTimers();
  ...
  it("never fires the deadline of a disposed channel", async () => {
    vi.useFakeTimers({ toFake: ["setTimeout", "clearTimeout"] });
    try {
      const channel = await newChannel(20);
  ...
  it("leaves a channel that is already ready alone when armed late", async () => {
    vi.useFakeTimers({ toFake: ["setTimeout", "clearTimeout"] });
    try {
      const channel = await newChannel(20),
        client = newClient(channel.path),
        reader = await authenticate(channel, client);
      // The handshake beat the owner's arm call; a deadline started now would
  ...
  it("leaves an idle-authenticated spare alone when armed late", async () => {
    vi.useFakeTimers({ toFake: ["setTimeout", "clearTimeout"] });
    try {
      const channel = await newChannel(20, true);
      await authenticateIdle(channel);
      channel.armReadyDeadline();
      expect(vi.getTimerCount()).toBe(0);
      // A spare the deadline tore down could no longer start a session.
      await expect(
        channel.start({
          columns: 80,
          command: ["cmd.exe"],
      ...
      // The session start arms a fresh deadline of the ready timeout.
      await vi.advanceTimersByTimeAsync(19);
      expect(vi.getTimerCount()).toBe(1);
      await vi.advanceTimersByTimeAsync(1);
      await expect(channel.ready).rejects.toMatchObject({ reason: "timeout" });
  ...
  it("arms a spare's ready deadline only after its host has spawned", async () => {
    vi.useFakeTimers({ toFake: ["setTimeout", "clearTimeout"] });
    let releaseSpawn: () => void = () => {};
    const spawnGate = new Promise<void>((resolve) => {
        releaseSpawn = resolve;
    ...
    try {
      ...
      const { idleAuthenticated } = fixture.channel(),
        rejection = rejectionOf(idleAuthenticated);
      // A slow source write or spawn is not the host being slow to answer.
      await vi.advanceTimersByTimeAsync(CONPTY_READY_TIMEOUT_MS * 1.5);
      expect(rejection()).toBe(PENDING);
      releaseSpawn();
      // Still short of the fallback, so only the armed deadline can fire.
      await vi.advanceTimersByTimeAsync(CONPTY_READY_TIMEOUT_MS);
      await expect(idleAuthenticated).rejects.toMatchObject({
        reason: "timeout",
      });
      await vi.waitFor(() => {
        expect(spawned[0]?.killed).toBe(true);
...
describe("ConPTY ready transition", () => {
  ...
  it("resolves the shell only against the live host's ready event", async () => {
    ...
    // The ready timeout starts with the host, not with the work before it.
    expect(control.armReadyDeadline).toHaveBeenCalledOnce();
    await pty.kill();
    await pty.onExit;
  });
  ...
  it.each([
    ...
    [
      "a file URL",
      (directory: string): URL | string => pathToFileURL(directory),
    ],
  ])(
    "hands the host %s working directory as an argument, never to the spawn",
    async (_kind, cwdOf) => {
      const host = testHost(),
        hostPid = liveHostPid(host),
        ready = Object.freeze(readyEvent(hostPid, hostPid + 1)),
        control = fakeControl(Promise.resolve(ready), () => host.kill()),
        dependencies = conPtyDependencies(control, host),
        // Need not exist: a missing directory must reach the host, which
        // reports it as a shell-start failure instead of a broken runtime.
        directory = join(tmpdir(), `missing-${randomUUID()}`),
        pty = constructConPty(dependencies, { cwd: cwdOf(directory) });

      await pty.shell;
      const [, args, options] =
        vi.mocked(dependencies.spawn).mock.calls[0] ?? [];
      ...
      expect(Object.keys(options ?? {})).toEqual(["env"]);
      await pty.kill();
      await pty.onExit;
    },
  );

  it("omits the working directory argument when none is given", async () => {
    const host = testHost(),
      hostPid = liveHostPid(host),
      ready = Object.freeze(readyEvent(hostPid, hostPid + 1)),
      control = fakeControl(Promise.resolve(ready), () => host.kill()),
    ...
    await pty.shell;
    const [, args, options] = vi.mocked(dependencies.spawn).mock.calls[0] ?? [];
...
describe("ConPTY source materialization", () => {
  ...
  it("shares one repair when concurrent opens find the cached source missing", async () => {
    const source = `# test-source ${randomUUID()}`,
      first = await CONPTY_DEPENDENCIES.materializeSource(source),
    ...
    try {
      await rm(first);
      vi.spyOn(fsPromises, "access").mockImplementation(async (...args) => {
        try {
          await access(...args);
        } finally {
          // Both callers must observe the deletion before either repairs it.
          if (++checks === 2) releaseChecks();
          await checksReady;
        }
      });
      const repairReads = vi.spyOn(fsPromises, "readFile"),
        repairs = await Promise.allSettled([
          CONPTY_DEPENDENCIES.materializeSource(source),
          CONPTY_DEPENDENCIES.materializeSource(source),
        ]);
      expect(repairReads).toHaveBeenCalledOnce();
      ...
      releaseChecks();
      await rm(first, { force: true });
      await rm(`${first}.${String(process.pid)}.tmp`, { force: true });
    }
  });
});
