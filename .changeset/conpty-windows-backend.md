...
- Add a Windows **Python executable** setting with automatic detection, interpreter status, and a download link when Python is missing. Profiles can specify their own interpreter. Resolved paths stay out of synced settings. Missing Python falls back to ConHost without its resizer; detecting Python on recheck or reload restores automatically demoted profiles to ConPTY.
...
- Retry ConPTY when restarting or restoring an automatically demoted terminal tab after Python becomes available, while preserving explicit ConHost choices.
