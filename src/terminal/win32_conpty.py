...
The plugin embeds this file as text, writes it to a temporary file, and runs::
    ...
    python <file> <columns> <rows> <pipe-name> [--cwd <dir>] -- <executable> [args...]
...
"""Native extensions searched before any batch launcher."""
_EXECUTABLE_EXTENSIONS = (".com", ".exe")
"""Batch extensions accepted only after the native search is exhausted."""
_BATCH_EXTENSIONS = (".bat", ".cmd")
...
def pump_output(
    read: Callable[[], bytes | memoryview],
    write: Callable[[bytes | memoryview], bool],
) -> None:
    """Forward chunks from ``read`` to ``write`` until ``read`` returns none.

    A failed write stops the forwarding, never the reading: ``ClosePseudoConsole``
    waits for conhost to flush into the output pipe, so a pump that left the
    pipe undrained would hang the close, and the host with it, forever.
    """
    forwarding = True
    while True:
        chunk = read()
        if not chunk:
            return
        if forwarding and not write(chunk):
            diagnose("stdout closed; discarding the remaining output")
            forwarding = False
...
def build_command_line(command: Sequence[str]) -> str:
    ...
    No shell is involved for a native executable, so nothing in any argument
    is expanded.  A ``.cmd`` or ``.bat`` target is the exception:
    ``CreateProcessW`` runs it through ``cmd.exe``, whose parser still
    interprets ``&``, ``|``, ``^``, and ``%`` inside these arguments, and this
    quoting does not protect against that.

    A NUL would silently end the command line early, so it is refused.
    """
    if not command or not command[0]:
        raise ValueError("a non-empty executable is required")
    if any("\x00" in argument for argument in command):
        raise ValueError("a command line cannot contain a NUL character")
    ...
    def feed(self, data: bytes) -> list[dict[str, Any]]:
        ...
        while True:
            ...
            if len(raw) > _MAX_CONTROL_LINE_BYTES:
                diagnose("dropped a control line over the size cap")
                continue
            try:
...
def parse_arguments(
    ...
    Immediate form:
    ``<columns> <rows> <pipe-name> [--cwd <dir>] -- <executable> [args...]``.
    ...
    options = head[3:]
    has_cwd = len(options) == 2 and options[0] == "--cwd"
    if len(head) < 3 or (options and not has_cwd):
        raise ValueError(
...
def resolve_executable(executable: str, path: str | None) -> str:
    ...
    ``_start`` applies the profile PATH to this host before calling, so the
    ...
    # An extension is a dot with something after it.
    if "." in program[:-1]:
...
def _parse_start(message: Mapping[str, Any], pipe_name: str) -> _StartRequest | None:
    ...
    return _StartRequest(
        ...
        # The token authenticates this host; no child may ever receive it.
        environment={
            key: value
            for key, value in (environment or {}).items()
            if key.upper() != TOKEN_ENVIRONMENT_VARIABLE
        }
        or None,
    )
...
if sys.platform == "win32":
    ...
    def _create_child(
        ...
        A deferred session passes the child environment explicitly and an
        immediate session inherits it from this host. Both inherit the host's
        working directory, which already reflects the session's ``cwd``.
        """
        size = _SIZE(0)
    ...
    def _exit_code(process: int) -> int:
        ...
        The wait result decides whether the child still runs: its exit code
        cannot, because a shell may legitimately exit with ``STILL_ACTIVE``.
        """
        if _WaitForSingleObject(process, 0) != _WAIT_OBJECT_0:
    ...
    class _ConPtyHost:
        ...
        def _run_session(self) -> int:
            ...
            try:
                ...
                # A ``ValueError`` is a NUL in the command or the directory:
                # the profile is at fault, exactly as for a refused shell.
                return (
        ...
        def _start(self) -> None:
            ...
            if cwd is not None:
                ...
                # one is a shell-start failure for both session kinds instead
                # of a failed host spawn. ``CreateProcessW`` also searches the
                # parent's directory before PATH, and relative program names
                # need it.
                try:
                    os.chdir(cwd)
                except OSError as error:
            ...
            def create_session() -> tuple[int, int, int]:
                ...
                path = environment_path(environment)
                if path is not None:
                ...
                return _create_child(
                    build_command_line(
                        (
                            resolve_executable(program, path),
                            *rest,
                        )
                    ),
                    self._pseudoconsole,
        ...
        def _pump_output(self) -> None:
            ...
            def read() -> memoryview:
                """Read one chunk; an empty one means the pipe ended."""
                if not _ReadFile(
            ...
            pump_output(read, lambda chunk: _write_all(_STDOUT_FD, chunk))
