...
export class TerminalTextArea implements IDisposable {
  ...
  public constructor(options?: TerminalTextArea.Options) {
    ...
    const {
      ...
      handler = ((): ((handled: boolean) => () => boolean) => {
        const handler0 = (handled: boolean) => (): boolean => {
            this.#sequence = false;
            return handled;
          },
          ...
          switch (datum) {
            ...
            case "\u007f": {
              if (line) {
                let width = 0;
                for (
                  let xx = cursorX - 1, cell0 = line.getCell(xx, this.#cell);
                  width <= 0 && cell0;
                ...
                if (width > 0) {
                  ...
                  const rest = line.translateToString(false, 0, lineWidth),
  ...
  async #sync(): Promise<void> {
    ...
    if (!lock.isBusy(TerminalTextArea.writeLock)) {
      throw new Error();
    }
    ...
    for (const width of this.#widths) {
      ...
      if (line && yy === cursorY) {
        ...
        for (
          let cell = line.getCell(cursorX, this.#cell);
...
export namespace CursoredText {
  ...
  export async function info(
    ...
    return deepFreeze({
      ...
      endX,
      lines: range(startBaseY, endBaseY + 1).map(
        (yy) =>
          active
            .getLine(yy)
            ?.translateToString(
