...
export class FollowThemeAddon implements ITerminalAddon {
	...
	public activate(terminal: Terminal): void {
		const update = (): void => {
			...
			if (
				typeof this.opts.enabled === 'function' &&
				!this.opts.enabled()
			) {
				return
			}
	...
	#computeTheme(): {
		...
		const bgVar = this.opts.bgVar ?? FollowThemeAddon.#DEFAULT_BG_VAR,
			...
			accentVar =
				this.opts.accentVar ?? FollowThemeAddon.#DEFAULT_ACCENT_VAR,

			// Resolve CSS variables to final, computed css color strings
			bgStr =
				FollowThemeAddon.#resolveCssColor(bgVar, body)?.trim() ?? '',
			fgVarStr =
				FollowThemeAddon.#resolveCssColor(fgVar, body)?.trim() ?? '',
			accentStr =
		...
		const explicitFg = this.#toRGBA(fgVarStr) ??
			this.#toRGBA(computedBodyColor),
			...
			minCursorContrast = this.opts.minCursorContrast ??
			...
			alpha = Math.min(
				...
				Math.max(
					0,
					this.opts.selectionAlpha ??
					FollowThemeAddon.#DEFAULT_SELECTION_ALPHA,
				),
			),
