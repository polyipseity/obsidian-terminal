...
/** Ready-timeout multiple that only bounds a channel nobody armed. */
export const CONPTY_UNARMED_READY_TIMEOUT_FACTOR = 3;
...
export interface ConPtyControlChannel {
  ...
  /** Tightens the ready deadline to the ready timeout. Called once the host
   * process has spawned, so the work before the spawn is not counted against
   * the host. */
  readonly armReadyDeadline: () => void;
...
export class WindowsNamedPipeControlChannel implements ConPtyControlChannel {
  ...
  #idleAuthed = false;
  /** Cleared once the current handshake phase has completed: a deadline armed
   * after that would only tear down a healthy channel. */
  ...
  private constructor(
    ...
    // immediate channel, spawn to idle authentication on a deferred one. It
    // starts as a generous fallback, so a channel its owner never arms still
    // cannot wait on a silent host forever.
    this.#armDeadline(readyTimeoutMs * CONPTY_UNARMED_READY_TIMEOUT_FACTOR);
  }

  /**
   * Tightens the pending deadline to the ready timeout. The owner calls it
   * after the spawn, so source materialization, the environment lookup, and
   * the spawn itself are not charged to the host. It does nothing once the
   * handshake phase has completed.
   */
  public armReadyDeadline(): void {
    if (this.#disposed || this.#readyDeadline === void 0) return;
    this.#armDeadline(this.#readyTimeoutMs);
  }
...
/** No `cwd`: the host takes the working directory as `--cwd` or in its
 * `start` op, so Node never fails the host spawn over a missing one. */
...
export const CONPTY_DEPENDENCIES: ConPtyPseudoterminalDependencies = {
  ...
  async materializeSource(source) {
    ...
    if (pending) {
      ...
      if (!present) {
        if (materializedConPtySources.get(source) === pending) {
          materializedConPtySources.delete(source);
        }
        // Another caller may have started repairing it during the access check.
        pending = materializedConPtySources.get(source);
      }
    }
    if (!pending) {
...
export class ConPtyPseudoterminal implements Pseudoterminal {
  ...
  public constructor(
     ...
     * spawn itself, which reads as a broken runtime.
     */
    const hostCwd = async (): Promise<string | null> =>
        cwd === void 0
          ? null
          : typeof cwd === "string"
            ? cwd
