...
function patchLoggingConsole(console: Console, log: Log): () => void {
  ...
  return around(console, {
    debug(next) {
      return consolePatch("debug", next);
    },
    error(next) {
      return consolePatch("error", next);
    },
    log(next) {
      return consolePatch("info", next);
    },
    warn(next) {
      return consolePatch("warn", next);
    },
  });
}

function patchLoggingWindow(self0: Window, log: Log): () => void {
  const onWindowError = (error: ErrorEvent): void => {
      log.logger
        .emit({
          data: error,
          type: "windowError",
